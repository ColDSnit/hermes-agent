...
class TestDisplayDedupe:
    ...
    def test_new_generation_copy_keeps_original_chronological_position(self, db):
        """A protected-tail copy inserted after a newer user message must not
        move the older logical turns below that message in the display read.

        This is the durable ``C, A, B`` regression: compaction copies retain
        their original timestamps but receive newer SQLite ids.
        """
        sid = "s1"
        db.create_session(sid, source="cli")
        db.append_messages_batch(
